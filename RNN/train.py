...
def train_and_evaluate(ticker, data_file, start_date, end_date, input_size, hidden_size, num_layers, output_size,
    ...
    for train_index, val_index in kf.split(X_train_full):
        ...
        model = SimpleRNN(input_size, hidden_size, num_layers, output_size, dropout, seq_length, device).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            print(f'Run {run}, Fold {fold}, RNN Epoch [{epoch + 1}/{num_epochs}], Loss: {val_loss}')
            #if counter >= patience:
